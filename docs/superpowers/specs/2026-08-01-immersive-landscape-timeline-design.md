...
The landscape detail will place a compact night-navigation overlay at the top of the timeline. It will expose the same date-picker action, date and duration content, previous/next actions, disabled states, accessibility labels, and swipe navigation semantics as the portrait header. The overlay will use a material/background treatment so controls remain legible over timeline content.
...
At accessibility Dynamic Type sizes, overlay text may wrap within a bounded region. If the entire landscape composition cannot fit at an extreme size, the detail region may scroll vertically, but the timeline receives a useful minimum height before scrolling is introduced. Text will not be globally capped as a substitute for responsive layout.

## Component Boundaries

- Extract the shared night-navigation content or behavior so portrait and landscape variants cannot drift in date formatting, picker behavior, actions, or accessibility metadata.
...
- Timeline gestures continue to take precedence within the timeline plot; overlay controls must not accidentally initiate timeline pan or zoom.
